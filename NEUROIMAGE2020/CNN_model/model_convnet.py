...
#read in parameters from file and override the defaults 
overwrite_params( dic_params_in, dic_params )

#Load the parameters into their respective variables
...
model.add( SeparableConv1D(filters, kernel_size=kernel, strides=1, padding='same', 
                data_format='channels_last', dilation_rate=1, 
                input_shape=(L, channels) ,
                depth_multiplier=depth, activation="relu", use_bias=True) )
model.add(MaxPooling1D(pool_size=2))

model.add( SeparableConv1D(filters*2, kernel_size=kernel, strides=1, padding='same', 
                data_format='channels_last', dilation_rate=1,                 
                depth_multiplier=depth, activation="relu", use_bias=True) )
model.add(MaxPooling1D(pool_size=2))


model.add( SeparableConv1D(filters*4, kernel_size=kernel, strides=1, padding='same', 
                data_format='channels_last', dilation_rate=1,                 
                depth_multiplier=depth, activation="relu", use_bias=True) )
model.add(MaxPooling1D(pool_size=2))

model.add( SeparableConv1D(filters*8, kernel_size=kernel, strides=1, padding='same', 
                data_format='channels_last', dilation_rate=1,                 
                depth_multiplier=depth, activation="relu", use_bias=True) )
model.add(MaxPooling1D(pool_size=2))

model.add( SeparableConv1D(filters*16, kernel_size=kernel, strides=1, padding='same', 
                data_format='channels_last', dilation_rate=1,                 
                depth_multiplier=depth, activation="relu", use_bias=True) )
model.add(MaxPooling1D(pool_size=2))
...
#%% Evaluate the correlation for the entire scans  
